...
public class Controller {
    ...
    public void onGalleryDownloaded(ArrayList<Picture> pictures)
    {
        ...
        HashMap<String, Integer> report = model.generateReport();
        model.close();
        bus.post(new Boolean(true)); //Notifies the view that pictures are ready.
        //Report printed in logcat
        Iterator<HashMap.Entry<String, Integer>> itrReport = report.entrySet().iterator();
        boolean headerPrinted = false;
        while (itrReport.hasNext())
        {
            Map.Entry<String, Integer> entry = itrReport.next();
            if (!headerPrinted)
            {
                Log.d("Report", "User\t\tPosts");
                Log.d("Report", "====\t\t=====");
                headerPrinted = true;
            }
            Log.d("Report", entry.getKey() + "\t\t" + entry.getValue());
        }
    }
